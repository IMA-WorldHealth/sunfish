...
async function runScheduledTask(schedule) {
  ...
  const record = schedules.findOne({ id: schedule.id });

  // update the schedule to say that the run is in progress
  record.runInProgress = true;
