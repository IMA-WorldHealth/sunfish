...
async function runScheduledTask(schedule) {
  ...
  try {
    ...
    const files = await crawler.downloadDashboardComponents(dashboards, { skipGraphs: true });
    ...
    eventer.emit('event', `Asking DHIS2 for the members of ${schedule.userGroup.displayName}`);

    // figure out which users we want to send to.
    const group = await api.userGroups.get(schedule.userGroupId);
    eventer.emit('event', `Found a total of ${group.data.length} members`);
    ...
    const names = users.data.map(user => `${user.displayName} (${user.email})`);

    eventer.emit('event', `The members of ${schedule.userGroup.displayName} are ${names.join(',')}.`);
    ...
    eventer.emit('event', `Composing an email with ${files.length} attachments.`);
